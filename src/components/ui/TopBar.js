...
import { getRoleById } from "@/lib/userDetails";
...
export default function TopBar({ pageTitle = null, overrides = {} }) {
  ...
  React.useEffect(() => {
    const fetchRole = async () => {
      if (user?.role) {
        const name = await getRoleById(user.role);
        setRoleName(name);
      }
    };
    fetchRole();
    ...
    <div className="w-full flex items-center justify-between px-15 pt-8 border-b border-transparent">
      ...
      <div className="flex items-center gap-4 bg-white py-2 px-4 rounded-md">
        <div className="text-xs flex gap-1 items-center">
          <strong>{user?.name || "..."}</strong> |<span className="text-gray-800 font-medium">{roleName || "..."}</span>
          <div className="bg-[#ABE7E5] rounded-3xl p-1"><CircleUserRound size={20} /></div>
        </div>
      </div>
    </div>
  );
