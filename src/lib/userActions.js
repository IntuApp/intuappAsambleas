...
import { doc, collection, addDoc, updateDoc, deleteDoc } from "firebase/firestore"; // <-- Agregamos addDoc
...
export async function deleteOperator(id) {
  if (!db) throw new Error("Database connection not available");
  try {
    // Aquí idealmente también deberías borrar las entidades asociadas si tu lógica de negocio lo exige
    await deleteDoc(doc(db, "user", id));
    return { success: true };
  } catch (error) {
    throw new Error(error.message || 'Error al eliminar el operador');
  }
}