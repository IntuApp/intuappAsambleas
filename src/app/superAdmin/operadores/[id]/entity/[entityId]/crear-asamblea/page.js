...
const CreateAssemblyPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (editAssemblyId) {
      ...
      // If assembly was finished, reset to 'create' status to allow restarting
      if (
      ...
      if (res.success) {
        ...
        router.push(
          `/superAdmin/operadores/${id}/entity/${entityId}/assembly/${editAssemblyId}`
        );
      } else {
      ...
      if (res.success) {
        ...
        router.push(
          `/superAdmin/operadores/${id}/entity/${entityId}/assembly/${res.id}`
        );
      }
    }
    setSubmitting(false);
    ...
    <div className="flex flex-col w-full min-h-screen bg-[#F8F9FB]">
      ...
      <div className="flex-1 p-8 w-full max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          ...
          <ChevronRight size={14} />
          <Link
            href={`/superAdmin/operadores/${id}/entity/${entityId}`}
            className="hover:text-gray-600 underline"
          >
            {entityName}
          </Link>
          <ChevronRight size={14} />
          <span className="text-gray-800 font-medium">Crear asamblea</span>
        </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              {[
                ...
                { id: "free_document", label: "Documento libre" },
              ].map((method) => (
                <label
                  key={method.id}
                  className={`flex-1 flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all ${
                          ...
                          <div className="flex items-center justify-center gap-3">
                            <span
                              className={`text-xs font-medium ${
                                isBlocked ? "text-gray-900" : "text-gray-400"
                              }`}
                            >
                              {isBlocked ? "Si" : "No"}
                            </span>
                            <button
                              ...
                              className={`relative w-11 h-6 rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${
                                isBlocked ? "bg-[#5b6ef0]" : "bg-gray-200"
                              }`}
                            >
                              <span
                                className={`inline-block w-4 h-4 transform bg-white rounded-full transition duration-200 ease-in-out translate-y-1 ${
