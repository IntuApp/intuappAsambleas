...
const ManageAssemblyPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!assemblyId) {
        ...
        router.push(`/superAdmin/operadores/${id}/entity/${entityId}`);
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    if (res.success) {
      ...
      router.push(
        `/superAdmin/operadores/${id}/entity/${entityId}/assembly/${assemblyId}`
      );
    } else {
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-[#F8F9FB]">
      <div className="hidden">
        <TopBar pageTitle={`Gestionar Asamblea - ${entityName}`} />
      </div>
